...
public:
   Array() //Non parametric constructor
   {
      size = 10;
      length = 0;
      A = new T[size];
   ...
   Array(int sz) // parametric constructor
   {
      size = sz;
      length = 0;
      A = new T[size];
...
void Array<T>::Rearrange()
{
   ...
   {
      while (A[i] < 0)
         i++; //used to sort the negative numbers at the starting
      while (A[j] >= 0)
         j--; //used to sort the positive numbers at the end
      if (i < j)
         swap(&A[i], &A[j]);
   }
}
